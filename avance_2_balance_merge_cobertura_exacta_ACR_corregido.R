...
#
#Correcciones aplicadas sobre avance_2_balance_merge_cobertura_exacta_ACR.R:
#  BUG 1: cell_suelo puede ser double en terra ≥ 1.6; se fuerza as.integer() al indexar lc_soil
#          y porcentaje_lc_dominante para evitar asignación silenciosa incorrecta.
#  BUG 2: lc_extraido$ID se limita a [1, nrow(soil_pixels)] con pmin/pmax antes de indexar
#          soil_pixels$cell_suelo, evitando NA silenciosos en píxeles de borde.
#  BUG 4: ylim del barplot de ΔS falla si ds_ymin == ds_ymax (rango cero); se garantiza un
#          rango mínimo de ±0.1 mm.
...
#### TABLA 1: PARÁMETROS DE COBERTURA VEGETAL (Avance 2, enunciado Tabla 1) ####
...
#NOTA: estos valores fueron generados específicamente para esta actividad académica
tabla1 <- data.frame(
...
#clases con IAF=0, h_veg=0, kcbtab=0 reciben ETc=0 (sin transpiración ni evaporación del modelo)


#### CONSTANTES DEL MODELO DUAL (enunciado Avance 2) ####
Fc_cte  <- 0.1   #fracción de cobertura fija para todos los píxeles (enunciado Avance 2)
...
#### FECHAS DE SIMULACIÓN (igual que en Calculo_ETo.R, ayudantía 6) #### Los datos del INIA son de 2020-05-01 a 2026-05-01
fecha_datos <- as.POSIXct("2022-11-11")  #inicio del período climático (idéntico a Calculo_ETo.R)
fecha_fin   <- as.POSIXct("2025-07-16")  #fin del período climático (idéntico a Calculo_ETo.R)
...
#### FUNCIONES ETo - PENMAN-MONTEITH FAO (basadas en Calculo_ETo.R, ayudantía 6) ####
...
#CORRECCIÓN respecto a Calculo_ETo.R: se usa velviento en m/s (no km/h); el script original
#convertía a m/s pero luego llamaba Penman con km/h, lo cual sobreestimaba ETo considerablemente
...
#CORRECCIÓN respecto a Calculo_ETo.R: se usa pmax en lugar de max (el original usa max escalar
#y requería mapply; aquí la función opera directamente sobre vectores diarios)
...
#CORRECCIÓN respecto a Calculo_ETo.R: se usa pmin en lugar de min (mismo motivo que kcmax_fn)
...
#función de respuesta al estrés hídrico de Feddes (modificada según enunciado Avance 2)
...
#### FUNCIONES DE MANEJO DE RASTER (idénticas a avance_1_v4.R) ####
...
#se lee el shapefile generado en avance_1_v4.R; contiene el polígono de 30 km² del área de trabajo
...
#tal como ocurre automáticamente en read.csv y es el estilo de Calculo_ETo.R)
names(INIA_raw) <- make.names(names(INIA_raw), unique = TRUE)

#convertir fechas y forzar columnas numéricas (igual que en Calculo_ETo.R)
...
#el cálculo sigue el esquema de Calculo_ETo.R con los parámetros de la estación Las Puentes
...
#velocidad del viento: convertir de km/h a m/s antes de usar en Penman (corrección bug Calculo_ETo.R)
...
INIA_diario$ETo_mm <- Penman_fn(pendiente      = INIA_diario$pendiente,
                                ...
                                velviento_ms   = INIA_diario$velviento_m_s,    #m/s (corregido)
                                es             = INIA_diario$es,
                                ea             = INIA_diario$ea,
                                Tmedia         = INIA_diario$Tmedia)
INIA_diario$ETo_mm <- pmax(INIA_diario$ETo_mm, 0)  #valores ligeramente negativos se reemplazan por 0

#filtrar el período de simulación (idéntico al subset de Calculo_ETo.R)
...
INIA_balance <- INIA_balance[1:n_dias_bal, ]  #garantiza exactamente 365 filas
...
#para cada clase presente en Tabla 1, se calcula la serie diaria de Tp y Ep (mm d⁻¹)
#utilizando la corrección de Kcb por IAF (kcbIAF_fn), Kc_max y ke de acuerdo al enunciado
#Fc = 0.1 y Fw = 1 fijos (enunciado); few = 0.9 constante; kr = 1; kcmin = 0.15

clases_unicas <- unique(tabla1$value)  #todas las clases de Tabla 1 (1 a 16)

#matrices de resultados: Tp y Ep en mm d⁻¹ [365 filas × 16 columnas (una por clase)]
...
for (j in seq_along(clases_unicas)) {
  cls     <- tabla1$value[j]
  IAF_j   <- tabla1$IAF[j]
  h_veg_j <- tabla1$h_veg[j]     #altura de vegetación (m); distinto del cabezal de presión h (cm)
  kcb_j   <- tabla1$kcbtab[j]

  if (IAF_j == 0 && h_veg_j == 0 && kcb_j == 0) {
    #clases sin vegetación (agua, glaciar, suelo desnudo, urbanización, etc.): ETc = 0
    Tp_mm_por_clase[, j] <- 0
    Ep_mm_por_clase[, j] <- 0

  } else {
    #Kcb ajustado diariamente por IAF, viento y HR mínima (kcbIAF_fn, vectorizada sobre 365 días)
    kcb_diario   <- kcbIAF_fn(kcmin  = kcmin,
                               IAF    = IAF_j,
                               kcbtab = kcb_j,
                               u2     = u2_bal,     #vector 365 valores (m/s)
                               HRmin  = HRmin_bal,  #vector 365 valores (%)
                               h_veg  = h_veg_j)

    #Kc máximo diario (también vectorizado mediante kcmax_fn con pmax)
    kcmax_diario <- kcmax_fn(u2    = u2_bal,
                              HRmin = HRmin_bal,
                              h_veg = h_veg_j,
                              kcb   = kcb_diario)

    #coeficiente ke de evaporación del suelo (vectorizado mediante ke_fn con pmin)
    ke_diario    <- ke_fn(kr     = kr,
                           kcmax  = kcmax_diario,
                           kcb    = kcb_diario,
                           few    = few_cte)  #few = 0.9 fijo (Fc=0.1, Fw=1)

    #transpiración y evaporación potenciales (mm d⁻¹)
    Tp_mm_por_clase[, j] <- ETo_mm_bal * kcb_diario  #Tp = ETo × Kcb
    Ep_mm_por_clase[, j] <- ETo_mm_bal * ke_diario   #Ep = ETo × ke
  }
}

#convertir Tp y Ep de mm a cm para usar en el balance hídrico (unidades de S son cm)
Tp_cm_por_clase <- Tp_mm_por_clase / 10
Ep_cm_por_clase <- Ep_mm_por_clase / 10
Pp_cm_bal       <- Pp_mm_bal / 10  #precipitación también en cm para el balance
...
#según enunciado Avance 2: parámetros equivalentes = promedio ponderado por espesor de horizonte
...
#Todo se puede ponderar? incluso parametros adimensionales?
...
#Según el enunciado, se debe identificar qué cobertura ocupa el mayor porcentaje en cada
#píxel del mapa de suelos. Por ello, se reemplaza el resampleo modal por el procedimiento
#de la ayudante: cada píxel de la grilla de suelo se convierte en polígono y se calcula la
...
#BUG 2 FIX: lc_extraido$ID es un índice de fila (base 1) en soil_pixels. Si terra genera IDs
#fuera del rango real por píxeles de borde, la indexación produce NA silencioso. Se limita al
#rango válido antes de indexar.
lc_extraido$ID <- pmin(pmax(as.integer(lc_extraido$ID), 1L), nrow(soil_pixels))
...
lc_soil <- ref_proj
values(lc_soil) <- NA
#BUG 1 FIX: cell_suelo puede ser de tipo double (dependiendo de la versión de terra);
#terra requiere índices integer para la asignación por posición. Se fuerza as.integer()
#en ambos rasters para garantizar indexación correcta sin asignación silenciosa incorrecta.
lc_soil[as.integer(lc_dominante$cell_suelo)] <- lc_dominante$landcover
...
porcentaje_lc_dominante[as.integer(lc_dominante$cell_suelo)] <- lc_dominante$porcentaje
...
lc_col_vec[is.na(lc_col_vec)] <- 1                  #clase sin match → columna 1 (Agua, ETc = 0)
...
S_current       <- S_init_vec          #almacenamiento actual [cm]; inicia en CC
stress_days_vec <- rep(0, n_pix)       #contador de días con estrés hídrico α < 1 por píxel
...
#identificar el índice del píxel seleccionado (centroide del polígono con cobertura vegetal)
#se selecciona el píxel más cercano al centroide del polígono que tenga clase con ETc > 0
centroide_pol  <- st_centroid(polygon)
centroide_proj <- st_transform(centroide_pol, crs = crs(ref_proj))
coords_cent    <- st_coordinates(centroide_proj)

#coordenadas de todos los píxeles válidos
xy_todos      <- xyFromCell(ref_proj, which(valid_mask))  #coordenadas [n_pix × 2]
dist_cent     <- sqrt((xy_todos[, 1] - coords_cent[1])^2 +
                      (xy_todos[, 2] - coords_cent[2])^2)  #distancia euclidiana al centroide

#filtrar píxeles con cobertura con ETc > 0 (clases con kcbtab > 0 en Tabla 1)
clases_con_etc <- tabla1$value[tabla1$kcbtab > 0 | tabla1$IAF > 0]
mask_veg       <- lc_vec %in% clases_con_etc  #TRUE si el píxel tiene vegetación con ETc > 0
dist_veg       <- dist_cent
dist_veg[!mask_veg] <- Inf  #ignorar píxeles sin vegetación en la búsqueda del centroide
idx_sel        <- which.min(dist_veg)  #índice del píxel seleccionado dentro de valid_mask

#FIX 1: which.min() retorna integer(0) si todos los valores son NaN (distancias no calculables)
#        En ese caso se usa el píxel 1 como fallback para evitar errores en cadena
if (length(idx_sel) == 0) {
  idx_sel <- 1L
  warning("No se pudo calcular distancias al centroide. Se usa el primer píxel válido como fallback.")
}

#FIX 2: clase_sel_val puede ser integer(0) o NA si idx_sel apuntaba fuera de rango;
#        se verifica longitud y NA antes del if() para evitar "argument is of length zero"
...
#estructura del gráfico basada en el código de visualización de Calculo_ETo.R (ayudantía 6)
...
#FIX 3: max(Pp_mm_bal) puede ser 0 (sin lluvia) o -Inf (todo NA), generando ylim=c(0,0) o
...
#BUG 4 FIX: si todos los días tienen el mismo ΔS (ds_ymin == ds_ymax), barplot falla con
#ylim de rango cero. Se garantiza un rango mínimo de ±0.1 mm en cualquier caso.
if (abs(ds_ymax - ds_ymin) < 0.1) { ds_ymin <- ds_ymin - 0.1; ds_ymax <- ds_ymax + 0.1 }
...
), col.names = c("Clase cobertura", "ETc acumulada (mm año⁻¹)"),
   caption = "ETc potencial acumulada durante el año por clase de cobertura (método dual Kcb)")
